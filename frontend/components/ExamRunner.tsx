...
/**
 * IMPORTANT: this component never sees or stores which option is correct.
 * `submitAnswer` is fire-and-forget from the UI's perspective — grading and
 * the "is this right?" feedback only ever happen server-side, after the
 * whole attempt is submitted (see `examApi.submitExam`).
 */
export function ExamRunner({ session, lang, onFinished }: ExamRunnerProps) {
  ...
  // The countdown shown to the trainee is UX only. The real enforcement is
  // `expires_at` on the attempt, checked by the backend on every /answer call.
  useEffect(() => {
  ...
  async function handleAnswer(optionId: string | null) {
    ...
    try {
      ...
      // Network hiccup — the backend still has the attempt; a resume/retry
      // flow belongs here in a fuller implementation.
    }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between text-xs text-azda-off-white/60 mb-3 px-1">
        <span className="rounded-full border border-azda-brass/40 bg-azda-brass/15 px-2.5 py-1 font-bold text-azda-brass-light">
          {session.subject_code} · L{session.level_index + 1}
        </span>
        <span className={`tabular-nums ${timeLeft <= 5 ? "text-azda-coral font-bold" : ""}`}>
          ⏱ {timeLeft}
        </span>
      </div>

      <div className="rounded-xl bg-azda-parchment text-azda-ink p-5 shadow-lg">
        <div className="text-[11px] font-bold uppercase tracking-wide text-azda-steel mb-2">
          ...
          {question.options.map((opt) => (
            <button
              ...
              className={`flex items-center justify-between rounded-[10px] border px-4 py-3 text-start text-[14.5px] transition-colors ${
                selected === opt.id
                  ? "border-azda-brass bg-white"
                  : "border-azda-parchment-dim bg-azda-off-white hover:border-azda-brass hover:bg-white"
