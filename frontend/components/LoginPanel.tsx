...
export function LoginPanel({ lang, onSubmit }: LoginPanelProps) {
  ...
  return (
    <div className="rounded-xl bg-azda-parchment text-azda-ink p-7 shadow-lg">
      ...
      {error && (
        <div className="mb-3 rounded-lg border border-azda-coral/35 bg-azda-coral/10 px-3 py-2 text-center text-xs text-[#7a2d1d]">
          {error}
        </div>
      )}
      ...
      <div className="flex justify-center mt-2">
        <button
          ...
          className="rounded-full bg-gradient-to-b from-azda-brass-light to-azda-brass px-8 py-3 text-sm font-bold text-azda-navy-deep shadow-md hover:-translate-y-0.5 transition-transform"
        >
          {s.start}
        </button>
      </div>
...
function Field({
  ...
  return (
    <div className="mb-3">
      <label className="block text-[11.5px] font-bold text-azda-steel mb-1.5">{label}</label>
      <input
        type={type}
        value={value}
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
        className="w-full rounded-lg border border-azda-parchment-dim bg-azda-off-white px-3.5 py-2.5 text-sm text-azda-ink focus:outline-none focus:border-azda-brass focus:bg-white"
      />
    </div>
  );
}
