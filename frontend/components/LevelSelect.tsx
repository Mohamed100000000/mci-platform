...
const DOT_COLOR = { easy: "#2f8f7c", medium: "#c8952f", hard: "#c1462f" };
...
export function LevelSelect({ levels, subjectCode, lang, onSelect, onBack }: LevelSelectProps) {
  ...
  return (
    <div className="rounded-xl bg-azda-parchment text-azda-ink p-6 shadow-lg">
      ...
      <div className="flex flex-col gap-2">
        {levels.map((lvl) => (
          <button
            ...
            className="flex items-center justify-between rounded-lg border border-azda-parchment-dim bg-azda-off-white px-4 py-3 hover:border-azda-brass hover:bg-white transition-colors"
          >
            <div className="flex items-center gap-2.5">
              <span
                className="flex h-6.5 w-6.5 shrink-0 items-center justify-center rounded-full text-xs font-bold text-azda-off-white"
      ...
      <div
        onClick={onBack}
        className="mt-3.5 text-center text-xs text-azda-off-white/50 underline cursor-pointer"
      >
        {s.back}
      </div>
    </div>
