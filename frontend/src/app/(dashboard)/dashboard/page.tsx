...
export default function DashboardPage() {
  ...
  const levelData = data
    ? Object.entries(data.scoreRangeCounts).map(([level, count]) => ({ level, count }))
    : [];
              ...
              {levelData.map((l) => (
                <div key={l.level} className="flex items-center justify-between text-xs">
                  <span className="flex items-center gap-1.5">
                    <span className="h-2 w-2 rounded-full" style={{ background: LEVEL_COLORS[l.level] || "var(--brand-400)" }} />
                    {l.level}
                  </span>
        ...
        <CardContent className="space-y-1">
          {isLoading ? (
            ...
            data.candidates.slice(0, 5).map((c) => {
              const mci = data.mciByTrainee[c.id];
              return (
                <div key={c.id} className="flex items-center gap-3 rounded-lg px-2 py-2.5 hover:bg-muted/50">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-xs text-muted-foreground">
                      {c.rank || "Rank not set"} &middot; {c.nationality || "—"}
                    </p>
                  </div>
                  {mci ? (
                    <Badge variant="accent">{mci.total_score.toFixed(0)} MCI</Badge>
                  ) : (
