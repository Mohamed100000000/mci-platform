...
import { ArrowLeft, Gauge, MapPin, Briefcase, Clock } from "lucide-react";
...
import { useSavedMci } from "@/hooks/use-mci";
...
export default function CandidateDetailPage() {
  ...
  const { data: mci, isLoading: mciLoading } = useSavedMci(id);
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-1">
          <CardContent className="flex flex-col items-center pt-6 text-center">
            ...
            <p className="text-sm text-muted-foreground">{candidate.current_position || "Position not set"}</p>
            ...
            <div className="mt-5 w-full space-y-3 text-left text-sm">
              ...
              <div className="flex items-center gap-2 text-muted-foreground">
                <Briefcase className="h-4 w-4" /> {candidate.current_position || "Not specified"}
              </div>
              <div className="flex items-center gap-2 text-muted-foreground">
                <Clock className="h-4 w-4" /> {candidate.years_of_experience} years experience
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Latest computed score and category breakdown.</CardDescription>
          </CardHeader>
          <CardContent>
            {mciLoading ? (
              ...
              <div>
                <div className="mb-6 flex items-center gap-6">
                  <div className="text-5xl font-semibold text-primary">{mci.mci_score.toFixed(0)}</div>
                  <div>
                    <Badge variant="accent">{mci.level}</Badge>
                ...
                <div className="space-y-3">
                  {mci.categories.map((c) => (
                    <div key={c.key}>
                      <div className="mb-1 flex justify-between text-xs">
                        <span className="font-medium">{c.label}</span>
                        <span className="text-muted-foreground">{c.sub_score_0_100.toFixed(0)} / 100</span>
                      </div>
                      <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
                        <div
                          className="h-full rounded-full bg-primary transition-all"
                          style={{ width: `${c.sub_score_0_100}%` }}
                        />
                      </div>
                    </div>
                  ))}
