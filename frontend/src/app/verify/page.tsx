...
import { examApi, type CertificateVerification } from "@/lib/exam/examApi";
...
export default function VerifyPage() {
  ...
  useEffect(() => {
    // Reads the verification code from the URL path once on mount, then
    // fetches its status from the backend. setCode/setData/setLoading are
    // synchronizing component state with this external (URL + network) input.
    const parts = window.location.pathname.split("/");
    const c = parts[parts.length - 1];
    // eslint-disable-next-line react-hooks/set-state-in-effect -- intentional, one-time URL read
    setCode(c);
    examApi
      .verifyCertificate(c)
      .then(setData)
