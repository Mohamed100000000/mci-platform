...
import { examApi, type ExamSession, type Question } from "@/lib/exam/examApi";
...
export function ExamRunner({ session, lang, onFinished }: ExamRunnerProps) {
  ...
  useEffect(() => {
    // Resets the per-question countdown/selection state whenever the
    // trainee moves to a new question — this is genuine synchronization
    // with the "current question index" external input, not derivable
    // state, so a direct setState here is the correct pattern.
    // eslint-disable-next-line react-hooks/set-state-in-effect -- intentional, resets timer on question change
