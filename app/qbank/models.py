...
# كما هي للتوافق العكسي. الـjob/task الجديدان طبقة أعلى تُنش٦ "دفعة" فعلية
...
class QbankQuestion(Base, TimestampMixin):
    ...
    embedding_status: Mapped[str] = mapped_column(String(20), nullable=False, default="not_computed") None"] = relationship()
...
class QbankMediaAsset(Base, TimestampMixin):
    """بيانات وصفية للملف فقط — الملف نفسه في تخزين كائنات خارجي (S3-compatible)،
...
class QbankTraineeQuestionHistory(Base):
    ...
    source_document: Mapped["QbankSourceDocument |
