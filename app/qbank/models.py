...
- هذا النظام منفصل تمامًا عن app/exam (الكويز/الاختبار القديم) ولا يستبدله
  منه ولا يعدّل عليه. app/exam يستخدم قاعدة تصريحية خاصة به وغير مرتبط
  بـتارget_metadata لأن app/models لا يستورده.
- عمود embedding (pgvector) مؤجَّل عمدًا لمرحلة لاحقة (كشف التكرار الدلالي
  عبر AI) تفادياً لإضافة اعتماد قاعدة بيانات ثقيل بدون حاجة مؤكدة بعد.
  عمود embedding_status نصي بسيط يوضح الحالة ("not_computed" الآن)، بحيث
  تكون إضافة العمود الحقيقي لاحقاً migration إضافية نظيفة بدون كسر شيء.
"""
import uuid
...
class QbankLearningObjective(Base, TimestampMixin):
    """هدف تعليمي مرتبط بكورس، ويمكن ربطه بربط اختياريًا بمعيار كفاءة موجود.

    ملاحظة مهمة: عدة أسئلة يمكن أن تشترك في نفس learning_objective_id عمدًا —
    هذا هو المقصود بـ"نسخ متخللفة من نفس الـcompetency بدون اعتباره أسئلة
    متطابقة". التكرار الحقيقي يُكتشف من مقارنة نص السؤال نفسه (content_hash /
    لاحقاً embedding) وليس من مجرد اشتراك عدة أسئلة في نفس الهدف التعليمي.
    """
...
class QbankGenerationBatch(Base, TimestampMixin):
    """سجلّ طلب توليد أسئلة بالـAI. الجدول موجود من (فارز عملياً) استعدادًا
...
class QbankSourceDocument(Base, TimestampMixin):
    ...
    النص الفعلي (أو مقتطف منه) الذي يُمرَّر لـAI كسياق عند التوليد — هذا هو
    الفصل هو ما يجعل التوليد "مؤسس على مصدر" بمعنى حقيقي وليس شكليًا.

    ملاحظة نطاق هذه المرحلة: استخراج النص من ملفات PDF/Word مؤجَّل — هذا
    الجدول يقبل نصًا جاهزًا حاليًا (مُلصَّق أو مُدخَل يدويًا) الآن. خط
    استخراج تلقائي من الملفات المرفوعة نقطة توسعة واضحة ولاحقة، ولايس
...
class QbankQuestion(Base, TimestampMixin):
    ...
    # كشف التكرار — الطبقة الأولى (تكرار حرفي/حرفي) فعّالة من الآن، بدون AI.
    # الطبقة الثانية (تكرار دلالي عبر embeddings) مؤجَّلة — انظر الملاحظة أعلى الملف
    # عن embedding_status ونطق يوضح الحالة ("not_computed" الآن)، بدلاً منه
    # تكوين إضافة العمود الحقيقي لاحقاً migration إضافية نظيفة بدون كسر شيء.
    ...
    # إحصائيات (تُحدَّث لاحقًا من محرك الامتحان الفعلي — أعمدة موجودة من الآن)
...
class QbankOption(Base):
    """خيار إجابة. يختلف الأنواع الأربعة:
    - MCQ / MULTIPLE_RESPONSE: is_correct هو المحدّد (أكثر من صف صحيح مسموح
      لـ multiple_response).
    - TRUE_FALSE: صفّان فقط (True/False)، is_correct يحدد الصحيح.
    - MATCHING: كل صف = زوج (text = العنصر الأيسر، match_text = طابقه
...
class QbankMediaAsset(Base, TimestampMixin):
    """بيانات وصفية لملف فقط — الملف نفسه في تخزين خارجي كائنات خارجي
    (S3-compatible)، غير مخزَّن داخل قاعدة البيانات."""
...
class QbankExamAttempt(Base):
    """محاولة امتحان حقيقية لمتدرب (منفصلة تمامًا عن ExamAttempt في
    app/exam القديم). لا تُنشأ ولا تُعدَّل أي أسئلة فعلية في هذه المرحلة -
    الجدول جاهز فقط لمرحلة محرك توليد امتحان قادمة."""
...
class QbankTraineeQuestionHistory(Base):
    """سجلّ مبسّط (متدرب، سؤال) لتسريع منع تكرار السؤال عند توليد امتحان
    جديد — بدون الحاجة لمسح كل محاولات الامتحان القديمة."""
